...
| L02 | CLI flags (model, yolo, work-dir, session, …) | AS-IS | `src/kimi_cli/cli/__init__.py` | `rki-rs/src/cli.rs`, `main.rs` | Partial | `cli::tests::*`, `tests/cli_integration.rs` | Flags: resume, session, yolo, auto-approve, model, work-dir, plan, ralph, print, archive, list-sessions, show-unified-events, unified-events-limit, unified-events-after, fork-from, acp, fork-context-up-to-id, **config**, **version**, **export-session**. |
...
| L06 | LLM / provider factory | AS-IS | `src/kimi_cli/llm.py` | `rki-rs/src/llm.rs`, `llm/openai.rs`, `llm/anthropic.rs` | Partial | `llm` module tests, `test_openai_provider_401_*`, orchestrator tests | Retry list matches tenacity set. OAuth 401 refresh: providers attempt `IdentityManager::refresh()` once on 401 and retry the request. |
...
| L11 | Soul construction | AS-IS | `src/kimi_cli/soul/kimisoul.py` | `rki-rs/src/soul.rs` (`KimiSoul`) | Done | `test_soul_run_*` (slash/plan/yolo/ralph), `test_soul_auto_title_after_turn`, `test_soul_rejects_empty_user_turn`, `test_soul_rejects_image_like_input_when_text_only`, `test_soul_echo_model_rejects_markdown_image_by_default`, `test_soul_accepts_multimodal_parts_when_vision_on` | ContextVar → explicit `ContextToken`. |
...
| L13 | Approval bridging to wire / UI | AS-IS | `run_soul`, `approval_runtime/` | `approval.rs`, `wire.rs` | Partial | `approval::tests::*` (capability engine, router, headless mode, sink priority, timeout) | Multi-sink router §6.3 style: shell → wire → background fallback. Capability engine with trust profiles. |
...
| L19 | Agent loop: MCP defer, max steps, StepBegin, compaction | AS-IS | `kimisoul.py`, `compaction.py` | `orchestrator.rs`, `compaction.rs`, `mcp/` | Partial | `orchestrator::tests::*`, `compaction`, `test_max_steps_limits_loop` | Step cap verified (max_steps limit enforced). Compaction thresholds covered by `compaction.rs` tests. |
...
| L24 | StatusUpdate (tokens, plan, MCP) | AS-IS | wire / soul | `wire.rs`, orchestrator | Partial | `test_status_update_fields_populated`, `test_status_update_reflects_plan_mode_after_plan_tool`, wire event tests | token_count, context_size, plan_mode, mcp_status all populated and verified. |
...
| S 2.2 | 2.2 | Wire taxonomy | AS-IS | `wire/types.py`, `protocol.py` | `wire.rs` | Partial | `wire` tests (20+ serde roundtrip tests) | TurnBegin/TurnEnd, StepBegin/StepInterrupted, SessionShutdown, TextPart/ThinkPart, Image/Audio/Video URL parts, ToolCall/ToolCallPart/ToolResult, ApprovalRequest/ApprovalResponse, PlanDisplay, Notification, SteerInput, MCP variants, CompactionBegin/End, BTW begin/end, HookTriggered/HookResolved all have serde tests. Golden fixtures: 32 events. |
...
| S 4.6 | 4.6 | Compaction | AS-IS | `soul/compaction.py` | `compaction.rs` | Partial | `compaction.rs` unit tests (6 tests) | Thresholds: noop when ≤preserve_count, summary on >preserve_count, summary count verified, empty history handled. |
