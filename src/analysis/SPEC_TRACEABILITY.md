...
**Last filled:** 2026-04-20 (L07 `workdir_ls`, ACP `RKI_ACP_TOKEN` bearer auth)
...
| L14 | Wire per turn + subscribers | AS-IS + DEV | `wire/` | `wire.rs` (`RootWireHub`, `Wire`, `WireEvent::SessionShutdown`) | Partial | `wire` tests, `MergedWireReceiver` tests, `test_session_shutdown_serde_roundtrip` | Unified stream §5.2 when feature flag on. |
...
| L27 | Grow context after LLM / tools | AS-IS | `context.py` | `context.rs`, orchestrator | Partial | orchestrator tests | |
| L28 | Tool rejected stop_reason | AS-IS | `kimisoul.py` | orchestrator / tools | Partial | orchestrator tests | |
